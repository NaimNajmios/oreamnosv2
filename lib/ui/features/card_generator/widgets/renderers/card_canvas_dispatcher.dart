...
class CardCanvasDispatcher extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return cardData.map(
      ...
      detailedScoreboard: (d) =>
          DetailedScoreboardCanvas(data: d, config: config),
      onThisDay: (d) => OnThisDayCanvas(data: d, config: config),
      startingXI: (d) => StartingXICanvas(data: d, config: config),
      matchStatsComparison: (d) =>
          MatchStatsComparisonCanvas(data: d, config: config),
